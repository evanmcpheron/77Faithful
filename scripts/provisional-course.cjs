...
const buildProvisionalCourse = (timestamp) => {
	if (manuscript.days.length !== 77 || manuscript.weeks.length !== 11) {
		throw new Error(
			'The provisional course needs exactly 77 days and 11 weeks.',
		);
	}
	...
	manuscript.weeks.forEach((week, index) => {
		if (
			...
			week.themeId !== FormationThemeOrder[index]
		) {
	...
	manuscript.days.forEach((day, index) => {
		...
		if (
			!verses?.length ||
			verses.some((verse) => !verse.verseLabel || !verse.text.trim())
		) {
			throw new Error(
...
const seedProvisionalCourse = async (database, timestamp) => {
	...
	const configurationReference = database.doc(
		'formationConfiguration/current',
	);
	return database.runTransaction(async (transaction) => {
		...
		if (
			...
			configuration.data().courseId !== courseId
		) {
			throw new Error(
				'Another course is configured. The provisional import will not replace it.',
			);
		}
		if (existingVersion.exists)
			return { courseId, courseVersionId, outcome: 'AlreadyImported' };
		...
		for (const [path, document] of documents)
			transaction.create(database.doc(path), document);
