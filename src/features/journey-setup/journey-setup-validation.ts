import { JourneySetupReadiness } from '@77/types/account/journey-setup.types';
import type {
  TJourneySetupChoices,
  TSetupOptionalPracticeSelection,
} from '@77/types/account/journey-setup.types';
import type { TReminderPreference } from '@77/types/account/device-preferences.types';
...
import type { TLocalClockTime } from '@77/types/shared/persistence.types';
...
export const getSetupChoices = (
  ...
  if (
    ...
    !selectedPractices.every((practiceId) => Object.values(OptionalPracticeId).includes(practiceId))
  ) {
    throw new Error('Choose different practices from the available choices.');
  }
  ...
  switch (selectedPractices.length) {
    ...
    case 1:
      ...
      break;
    case 2:
      optionalPracticeIds = [selectedPractices[0], selectedPractices[1]];
      break;
    case 3:
      optionalPracticeIds = [selectedPractices[0], selectedPractices[1], selectedPractices[2]];
      break;
    case 4:
      optionalPracticeIds = [
        selectedPractices[0],
  ...
  if (optionalPracticeIds.length !== 0 && optionalPracticeIds.length !== 1 && bibleVersionId) {
    return { readiness: JourneySetupReadiness.ReadyForReview, optionalPracticeIds, bibleVersionId };
  }
  return { readiness: JourneySetupReadiness.Incomplete, optionalPracticeIds, bibleVersionId };
};
