...
export const completeJourneyPracticeForAccount = async (
  ...
  return database.runTransaction(async (transaction) => {
    ...
    const previousOperation = (await transaction.get(operationReference)).data();
    const fingerprint = JSON.stringify(request);
    if (previousOperation) {
      if (previousOperation.fingerprint !== fingerprint)
        throw new HttpsError('already-exists', 'Reopen the day before making another change.');
      return previousOperation.result as ISetPracticeCompletionResult;
    }
    const { day, dayReference } = await readJourneyDay(transaction, database, userId, request);
    ...
    if (!currentCompletion)
      throw new HttpsError('failed-precondition', 'This practice isn’t assigned to this day.');
    ...
    const updateOptional = (practice: TAssignedOptionalPractices[number]) => ({
      practiceId: practice.practiceId,
      completion: practice.practiceId === request.practiceId ? completion : practice.completion,
    });
    ...
    const optionalPractices: TAssignedOptionalPractices =
      assigned.length === 4
        ...
        : assigned.length === 3
          ? [updateOptional(assigned[0]), updateOptional(assigned[1]), updateOptional(assigned[2])]
          : [updateOptional(assigned[0]), updateOptional(assigned[1])];
...
export const saveJourneyReflectionForAccount = async (
  ...
  return database.runTransaction(async (transaction) => {
    ...
    if (previousOperation) {
      if (previousOperation.fingerprint !== fingerprint)
        throw new HttpsError('already-exists', 'Reopen your reflection before saving again.');
      return previousOperation.result as ISaveWritingResult;
    }
    const { day, dayReference } = await readJourneyDay(transaction, database, userId, {
      ...request.target,
      observedPhoneTimeZoneId: request.observedPhoneTimeZoneId,
    });
    if ((day.reflection?.revisionId ?? null) !== request.expectedRevisionId) {
      throw new HttpsError(
        'aborted',
    ...
    const currentWriting = { revisionId, text: request.text, updatedAt: now };
    transaction.create(database.doc(`${journeyPath}/writingRevisions/${revisionId}`), revision);
    transaction.set(dayReference, {
      ...day,
    ...
    const result: ISaveWritingResult = {
      ...
      currentWriting: { ...currentWriting, updatedAt: serializeTimestamp(now) },
      conflictId: null,
    };
    transaction.create(operationReference, { fingerprint, result });
    return result;
