...
4. an unresolved product decision that must not be silently decided.

Do not treat Planned, Future, Optional / undecided, and Existing routes as interchangeable.
...
8. Check the navigation contract's Open Product Decisions before implementing anything affected by them.
...
Missing integration-backed guards must be reported as deferred integration work rather than represented as production-complete authorization.
...
Replace starter Home/Explore concepts with Today/Journey.
...
## Optional and Future routes

Do not create Optional / undecided routes unless the task explicitly enables the corresponding feature or the product decision has been resolved.

In particular, do not scaffold merely because they appear in the contract:

- `/auth/verify-email` while its gate placement remains unresolved,
- `/settings/notifications` when notifications are not implemented.

Do not create Future Community or invitation routes during V1 navigation work.
...
Do not create speculative tests for authentication, journey state, Firebase, API.Bible, Community, or Optional features that are not implemented.
...
10. Check that no Community or optional dead navigation is exposed.
