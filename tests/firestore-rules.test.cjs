...
const add = (
  ...
  tests.push({
    ...
    test: {
      ...
      functionMocks: [
        { function: 'exists', args: [{ anyValue: {} }], result: { value: profileExists } },
      ],
    },
  });
};
...
for (const method of ['get', 'list']) {
  ...
  add(`unverified journey ${method}`, 'DENY', method, journey, 'owner', false);
}
for (const method of ['create', 'update', 'delete']) {
  add(`owner journey ${method}`, 'DENY', method, journey, 'owner', true, true, {
    userId: 'owner',
    state: { status: 'Active' },
  });
}
add('journey subcollections remain private', 'DENY', 'get', `${journey}/days/1`);
for (const path of [draft, device, `${draft}/writingRevisions/12345678901234567890`]) {
  ...
  for (const method of ['create', 'update', 'delete'])
    add(`other account ${method} ${path}`, 'DENY', method, path, 'other', true, true, {
      userId: 'other',
    });
  add(`owner delete ${path}`, 'DENY', 'delete', path);
  add(`missing fields create ${path}`, 'DENY', 'create', path, 'owner', true, true, {
    userId: 'owner',
  });
}
add('list setup drafts', 'DENY', 'list', 'users/owner/journeySetupDrafts');
add('alternate draft', 'DENY', 'get', 'users/owner/journeySetupDrafts/alternate');
add('invalid device id', 'DENY', 'get', 'users/owner/devicePreferences/invalid');
...
add('first setup create', 'ALLOW', 'create', draft, 'owner', true, true, newDraft);
tests.at(-1).test.request.time = testTimestamp;

const addWrite = (name, expectation, path, document, previousDocument, mocks) => {
  add(
    name,
    expectation,
...
addWrite('three practices create', 'ALLOW', draft, {
  ...readyDraft,
  choices: { ...readyDraft.choices, optionalPracticeIds: ['Movement', 'Gratitude', 'Worship'] },
});
const previousDraft = {
  ...readyDraft,
...
const updatedDraft = { ...readyDraft, revision: 1, createdAt: previousDraft.createdAt };
addWrite('four practices update', 'ALLOW', draft, updatedDraft, previousDraft);
const invalidDrafts = [
  [
    'duplicate practices',
    { choices: { ...readyDraft.choices, optionalPracticeIds: ['Movement', 'Movement'] } },
  ],
  [
    'five practices',
    {
      choices: {
        ...readyDraft.choices,
        optionalPracticeIds: ['Movement', 'Gratitude', 'Worship', 'Generosity', 'ChristianReading'],
      },
    },
  ],
  [
    'unknown practice',
    { choices: { ...readyDraft.choices, optionalPracticeIds: ['Movement', 'Unknown'] } },
  ],
  ['wrong readiness', { choices: { ...readyDraft.choices, readiness: 'Incomplete' } }],
  ['ready with no translation', { choices: { ...readyDraft.choices, bibleVersionId: null } }],
  ['unknown translation', { choices: { ...readyDraft.choices, bibleVersionId: 'Unknown' } }],
...
for (const [name, patch] of invalidDrafts) {
  ...
  addWrite(`${name} update`, 'DENY', draft, { ...updatedDraft, ...patch }, previousDraft);
}
addWrite('revision replay', 'DENY', draft, { ...updatedDraft, revision: 0 }, previousDraft);
addWrite(
  'createdAt changed',
  'DENY',
  draft,
...
addWrite(
  ...
  'ALLOW',
  device,
  { ...preferences, revision: 1, morningReminder: { isEnabled: false, localTime: '07:00' } },
  preferences,
);
...
addWrite('create linked writing revision', 'ALLOW', writingPath, writing, null, writingMocks);
addWrite(
...
const tombstoneWriting = { ...writing, baseRevisionId: existingHead.revisionId, text: null };
const tombstoneDraft = { ...updatedDraft, startingMotivation: { ...head, text: null } };
addWrite(
  ...
  [profileMock, mock('exists', false, writingPath), mock('getAfter', { data: tombstoneWriting })],
);
addWrite('create tombstone revision', 'ALLOW', writingPath, tombstoneWriting, null, [
  profileMock,
  mock('exists', true, draft),
  mock('get', { data: { ...previousDraft, startingMotivation: existingHead } }),
...
(async () => {
  ...
  const result = await client.post(
    ...
    {
      source: {
        files: [{ name: 'firestore.rules', content: fs.readFileSync('firestore.rules', 'utf8') }],
      },
