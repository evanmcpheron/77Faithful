...
import type { IDevicePreferencesDocument } from '@77/types/account/device-preferences.types';
import type { IJourneySetupDraftDocument } from '@77/types/account/journey-setup.types';
import { WritingKind } from '@77/types/journey/journey-writing.types';
import type {
  IWritingHead,
  IWritingRevisionDocument,
} from '@77/types/journey/journey-writing.types';
...
export const loadJourneySetup = async (
  ...
  return runTransaction(db, async (transaction) => {
    ...
    return {
      draft: draftSnapshot.exists() ? (draftSnapshot.data() as IJourneySetupDraftDocument) : null,
...
export const saveJourneySetup = async (input: ISaveJourneySetupInput): Promise<void> => {
  const draftReference = doc(db, 'users', input.userId, 'journeySetupDrafts', 'current');
  const deviceReference = doc(db, 'users', input.userId, 'devicePreferences', input.deviceId);
  const writingReference = doc(collection(draftReference, 'writingRevisions'));
  ...
  await runTransaction(db, async (transaction) => {
    ...
    const devicePreferences = deviceSnapshot.data();

    if (
      (draft?.revision ?? null) !== input.expectedDraftRevision ||
      (devicePreferences?.revision ?? null) !== input.expectedDeviceRevision
    ) {
      throw new JourneySetupConflictError();
    }

    const previousWriting: IWritingHead | null = draft?.startingMotivation ?? null;
    const hasMotivationChanged = motivation !== (previousWriting?.text ?? null);
    const startingMotivation: WithFieldValue<IWritingHead> | null = hasMotivationChanged
      ? { revisionId: writingReference.id, text: motivation, updatedAt: serverTimestamp() }
      : previousWriting;

    if (hasMotivationChanged) {
      const writingRevision: WithFieldValue<IWritingRevisionDocument> = {
        userId: input.userId,
        target: { kind: WritingKind.SetupMotivation, setupDraftId: 'current' },
        baseRevisionId: previousWriting?.revisionId ?? null,
        text: motivation,
        origin: { operationId: writingReference.id, deviceId: input.deviceId, recordedOnDeviceAt },
