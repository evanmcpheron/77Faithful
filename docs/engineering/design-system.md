...
`Typography` exports ordinary React Native text styles; `TypographyRole` defines [ThemedText](../../src/components/themed-text.tsx)'s `type` values. Non-Text primitives such as TextField reuse `Fonts.sans` and `Typography.body` without copying size values.
...
### Final component inventory and boundaries

The inventory below is the existing library, not a claim that its data-dependent consumers are integrated. Keep the small public components that express a current V1 responsibility; internal adapters are implementation details, not an additional public catalog.

| Area                     | Public components                                                                            | Implementation / usage boundary                                                                                                                         |
| ------------------------ | -------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Foundation               | ThemedText, ThemedView                                                                       | Use theme colors and typography through `useTheme` and `theme.ts`.                                                                                      |
| Actions                  | Button, IconButton, TextLink, AppIcon                                                        | One internal ActionControl owns action behavior. Use AppIcon's default 24-point glyph; the 20-point completion check fits its indicator.                |
| Surfaces/navigation rows | Surface, NavigationRow, Separator                                                            | Settings uses NavigationRow directly. Use its card appearance for a single destination; use a passive Surface for sibling actions.                      |
| Selection/completion     | CompletionControl, SelectionCard, RadioRow, SwitchRow                                        | Internal SelectionControl/SelectionTarget share checkbox/radio behavior; SettingSwitch preserves platform switch behavior.                              |
| Fields                   | TextField, PasswordField                                                                     | One field owner with an internal FormTextInput web measurement adapter. PasswordField adds only reveal state and its action.                            |
| Layout                   | ScreenScrollView, ScreenSection, ScreenHeading                                               | One screen scroll root; ordinary sections and headings inside it.                                                                                       |
| Feedback                 | InlineNotice, ErrorState, EmptyState, LoadingPlaceholder, SyncStatus                         | Internal FeedbackText handles state text/announcements. ErrorState adds only a real retry action; EmptyState gives explanatory headings a semantic API. |
| Daily composition        | PracticeCard, PrayerPrompt, MorningIntention                                                 | Compose existing primitives with caller-supplied approved content and private state. No duplicate card, editor, or completion implementation.           |
| Focused screens          | ScriptureReader, ReflectionEditor                                                            | Each owns its one scroll root. Keep reader-specific measure/verse layout and reflection submission composition here.                                    |
| Journey                  | DayProgress, PracticeProgress, JourneyProgress, DayStatus, JourneyDayRow, JourneyWeekSection | Text-first progress and content-supplied week/day presentation. Upcoming remains passive.                                                               |
| Navigation adapters      | AppTabs (native/web), SettingsHeaderAction, ExternalLink                                     | Preserve platform routing, back behavior, and external-browser handling. The web TabButton stays local to AppTabs.                                      |
| Identity/startup         | BrandWordmark, AnimatedSplashOverlay                                                         | Wordmark is the small introduction/end signature. The native splash overlay is startup infrastructure; its web export is a no-op.                       |

Do not add a SettingsRow, Card, field wrapper, or completion wrapper around an existing owner. ActionControl and SelectionControl remain separate: their navigation/action and checked/group interaction contracts differ. Small semantic adapters such as EmptyState and PrayerPrompt are intentionally retained; reducing file count alone is not a reason to replace their named responsibilities with repeated caller markup. No extra boolean modes, new public APIs, dependencies, routes, or generic form/list framework were needed in this audit.

Keep route composition, the web TabButton, ScriptureReader's reading layout, and component-specific indicator geometry local. Matching numbers do not establish shared design semantics: the reader's 640-point measure, 24-point selection indicator, radio circle/dot geometry, and static skeleton proportions do not need new global tokens or substitution with unrelated spacing/surface tokens. Flex factors, zero offsets, percentages, and animation keyframe/timing values are structural values. AppIcon owns the shared default icon size; callers need not repeat it. Startup uses the supplied [light](../../assets/splash-light.png) and [dark](../../assets/splash-dark.png) artwork, selected by system appearance. The native overlay fills the screen with centered cover scaling; its fallback backgrounds match the splash plugin. iOS uses Expo SDK 57's `enableFullScreenImage_legacy` option with cover scaling; revisit this option when upgrading Expo because it is scheduled for removal. Android uses a contained image in the system splash before the full-screen overlay. The overlay waits for image display (or image failure), layout, and Auth readiness before hiding the native splash; web remains a no-op.
...
Rows use the existing 48-point minimum target, grow with wrapped/scaled text, and introduce no font-scaling cap, fixed height, clipping, or animation. Pressed feedback uses `backgroundSelected`; disabled rows block activation, expose disabled state, remove the web destination, and use the disabled color pair. Provide a visible reason when needed. Keyboard focus uses the existing two-point outline and neutral offset. Leading icons and disclosure glyphs use AppIcon's hidden decorative subtree. The fill and focus target share `Radius.control` for rows and `Radius.surface` for cards; apply rounding to the painted child as well as the outer target, without clipping its contents.
...
Use simple recognizable platform icons with accessible names for icon-only actions; decorative icons stay hidden from assistive technology. Pair state color with text/icons and native accessibility state. Avoid animation-dependent meaning and honor reduced motion when introducing product transitions. The active native splash overlay uses the supplied light/dark artwork; unused AnimatedIcon exports and their glow/gradient CSS have been removed. Reanimated 4.5.1 keyframes default to `ReduceMotion.System`; verify actual overlay dismissal with reduced motion on a native release build. There are no expandable product controls or completion celebrations in the current inventory.
...
## Focused daily presentation

[ScriptureReader](../../src/components/scripture-reader.tsx) and [ReflectionEditor](../../src/components/reflection-editor.tsx) each own one `ScreenScrollView` for the existing pushed daily route; render them directly under the stack, without another scroll container. Route entry, access gates, standard Back, and unsaved-draft departure protection remain the caller's responsibility under [Scripture Reader](../APP_NAVIGATION_AND_UX.md#1014-scripture-reader) and [Reflection](../APP_NAVIGATION_AND_UX.md#1015-reflection). The routes remain scaffolds until approved content, journey state, and private persistence are connected.

ScriptureReader accepts the assigned reference and a `ScriptureResult` from the service. Ready results own the actual translation label, attribution, and ordered verse text; optional `unavailableTranslation` carries service-supplied reader metadata only when text is unavailable. It imports no content or runtime loader. Reading uses system body typography (17/26), a 640-point maximum measure, 24-point verse spacing, and chapter/verse coordinates without altering the supplied text. Unavailable text retains the reference and manual completion action. Completion is controlled, reversible, and never inferred from opening, scrolling, or translation changes. The current service has no retryable loading state, so this presentation adds no retry or artificial spinner.

ReflectionEditor composes TextField, Button, CompletionControl, and SyncStatus. `privateResponse` and `onPrivateResponseChange` are private draft data; callers must never send them to diagnostics. Required `draftStatus` distinguishes unsaved/saving from confirmed persistence outcomes. `onSaveAndComplete` must persist the response before recording completion; the component reports neither operation as successful itself. The separate `onSavePrivateResponse` supports later edits, including saving cleared text, while completion remains independently controlled. `saving` describes an actual explicit submission; `completion.pending` distinguishes a local write from pending sync. Local submission blocks conflicting interaction, while a sync backlog remains editable. Failure feedback uses fixed content-free wording. No sharing, persistence, navigation, or autosave timer is implemented here.

[Focused daily tests](../../src/components/focused-daily.test.tsx) use synthetic non-Scripture and private-response fixtures to verify explicit/reversible completion, unavailable text, service labels and attribution, draft outcomes, editing/clearing, pending writes, failures, and scrolling/scaling props. Existing TextField web growth and ScreenScrollView platform tests cover the reused primitives. Native keyboard occlusion, largest-text layout, and screen-reader delivery still need device verification.
...
Feature flows should consume these controls as their integrations are implemented. Configured iOS/Android icons point to the product PNG; platform-ready icon derivatives and the web favicon still need separate branding work. The configured splash and native overlay now use the supplied light/dark artwork.

## Final cohesion audit — 2026-09-08

Source review covered every component and platform adapter in `src/components`, all V1 route compositions, theme tokens, and adjacent tests. The bounded corrections align NavigationRow's painted corners with its target, remove redundant action icon sizing, and queue simultaneous iOS field-error announcements. Unused native/web AnimatedIcon implementations and their CSS were removed; the active splash overlay is retained. No duplicate production Button, card, text-field, Settings-row, or completion implementation required consolidation. Current participation copy, summaries, and states introduce no guilt, rankings, missed-day alerts, completion celebration, or spiritual scoring; failure wording describes storage/network operations.

An isolated Chrome preview exercised all 24 V1 screen paths in light/dark at 320-pixel width and 200% CSS zoom, checking headings, shared link targets, horizontal overflow, and Welcome keyboard focus/activation. Separate temporary synthetic fixtures exercised production controls and focused readers/editors at 320-pixel width with doubled text sizes/line heights. They checked wrapped actions, at least 48-point interactive targets, textarea growth after layout measurement, one overflowing scroll container, checkbox reversal with Space, radio arrow selection, switch keyboard activation, and passive named Upcoming rows in Chrome's accessibility tree. Fixture packaging used the installed React Native Web/components and a throwing stub for an unused server-only font context; it did not mock icons or supply production content. These are browser stress checks, not system Dynamic Type or VoiceOver/TalkBack evidence.

Final checks passed: `npm run check` (372 tests across 23 suites, formatting, lint, app/backend types, Auth synthesis, and the draft Scripture audit), `npm run export:web` (517 static routes), and the targeted TextField/Surface suites (29 tests). Existing React Native Web `pointerEvents` and CDK deprecation warnings remain; the unpublished Scripture audit does not establish release content readiness.

Native acceptance is still outstanding:

- **iOS:** largest accessibility text sizes, VoiceOver reading/focus order and queued field errors, multiline caret/Save visibility with the keyboard, actual top/bottom insets, native tabs, back/swipe behavior, and splash dismissal with reduced motion. A booted iPhone 16e simulator was found, but the development app was not installed. The [testing guide](testing.md) records the existing Xcode/ExpoModulesJSI build blocker; no native build was attempted for this audit.
- **Android:** largest font/display size, TalkBack names/states and live-region announcements, real switch hit slop, keyboard resize/focused-child scrolling, system Back, and tab/navigation-bar insets. Android SDK/device tooling was unavailable. Source inspection of installed Expo Symbols 57.0.2 found glyph Text using default font scaling inside a fixed-size View; verify Settings/check/disclosure glyphs at maximum text size for clipping. This is an unverified native layout concern, not an observed device failure; do not disable product text scaling or patch the dependency without reproduction.
- **Startup identity:** the configured splash and native overlay now use the supplied light/dark artwork; the starter web favicon still needs separate artwork work. Keep them out of the product component catalog. Test the final splash in a release build as required by the [Expo SDK 57 splash guidance](https://docs.expo.dev/versions/v57.0.0/sdk/splash-screen/).
