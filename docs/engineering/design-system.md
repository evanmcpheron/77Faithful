...
Keep route composition, the web TabButton, ScriptureReader's reading layout, and component-specific indicator geometry local. Matching numbers do not establish shared design semantics: the reader's 640-point measure, 24-point selection indicator, radio circle/dot geometry, and static skeleton proportions do not need new global tokens or substitution with unrelated spacing/surface tokens. Flex factors, zero offsets, percentages, and animation keyframe/timing values are structural values. AppIcon owns the shared default icon size; callers need not repeat it. The retained splash's `#208AEF` and image dimensions match existing startup artwork/configuration and remain an explicit branding exception until that artwork is replaced together.
...
Feature flows should consume these controls as their integrations are implemented. Configured iOS/Android icons point to the product PNG; platform-ready icon derivatives, the active starter splash overlay/configured splash and web favicon still need separate branding work. No artwork or Expo configuration changes are included here.
