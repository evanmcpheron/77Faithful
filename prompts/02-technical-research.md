...
Research only. Read `AGENTS.md`, relevant decisions, and the involved source/configuration. Check installed versions rather than treating the context inventory as permanent.
...
Cross-check the recommendation against current configuration and requirements. Recommend one approach with decisive tradeoffs and evidence limits; a comparison table is useful only when alternatives merit comparison. If uncertainty requires an experiment, describe a bounded experiment and what result would resolve it. Stop once the question is answered or the missing evidence is identified; do not install packages or build the experiment as part of research.
