...
const createSetupService = () => {
		...
		runTransaction: async (_database, callback) => {
			...
			const result = await callback({
				...
				set: (reference, document) =>
					writes.push([reference.path, document]),
			});
			if (shouldFailCommit) throw new Error('Connection unavailable');
			for (const [documentPath, document] of writes)
	...
	const loadModule = (filename) => {
		...
		const importModule = (specifier) => {
			...
			if (specifier === '@td/services/firebase/firebase.instance')
				return { db: {} };
			if (specifier.startsWith('@td/'))
				return loadModule(
					path.join(root, 'src', `${specifier.slice(4)}.ts`),
				);
	...
	return {
		service: loadModule(
			path.join(
				root,
				'src/features/journey-setup/journey-setup.service.ts',
			),
		),
		validation: loadModule(
			path.join(
				root,
				'src/features/journey-setup/journey-setup-validation.ts',
			),
		),
		documents,
		failCommit: () => {
...
test('saves and restores all choices, motivation and per-device reminder preferences', async () => {
	...
	const snapshot = await service.loadJourneySetup(
		initialInput.userId,
		initialInput.deviceId,
	);
	assert.deepEqual(snapshot.draft.choices, initialInput.choices);
	assert.equal(snapshot.draft.currentStep, 'Review');
	assert.equal(
		snapshot.draft.startingMotivation.text,
		initialInput.motivation,
	);
	assert.deepEqual(
		snapshot.devicePreferences.morningReminder,
		initialInput.morningReminder,
	);
	assert.deepEqual(
		snapshot.devicePreferences.eveningReflectionReminder,
		initialInput.eveningReflectionReminder,
	);
	const otherDevice = await service.loadJourneySetup(
		initialInput.userId,
		'anotherdevice',
	);
	assert.equal(otherDevice.devicePreferences, null);
	assert.deepEqual(otherDevice.draft, snapshot.draft);
	const otherAccount = await service.loadJourneySetup(
		'another-owner',
		initialInput.deviceId,
	);
...
test('saving another choice preserves the motivation head and does not add a writing revision', async () => {
	...
	assert.deepEqual(
		nextDraft.startingMotivation,
		firstDraft.startingMotivation,
	);
...
test('clearing motivation writes a tombstone linked to the previous revision', async () => {
	...
	const revision = documents.get(
		`${draftPath}/writingRevisions/${head.revisionId}`,
	);
...
test('stale drafts and stale device preferences reject without overwriting saved decisions', async () => {
	...
	await assert.rejects(
		service.saveJourneySetup({
			...initialInput,
			motivation: 'Stale writing',
		}),
...
test('a failed transaction does not save a partial setup', async () => {
	...
	failCommit();
	await assert.rejects(
		service.saveJourneySetup(initialInput),
...
test('partial setup choices remain incomplete and duplicate practice selections are rejected', () => {
	...
	assert.equal(
		validation.getSetupChoices(['Movement'], 'Web').readiness,
		'Incomplete',
	);
	assert.equal(
		validation.getSetupChoices(['Movement', 'Gratitude'], null).readiness,
		'Incomplete',
	);
	assert.equal(
	...
	assert.throws(() =>
		validation.getSetupChoices(['Movement', 'Movement'], 'Web'),
	);
});
...
test('a profile without setup documents loads an empty setup and can save its first choices', async () => {
	const { service } = createSetupService();
	assert.deepEqual(
		await service.loadJourneySetup('owner', initialInput.deviceId),
		{
			draft: null,
