...
const { getJourneyDayForAccount } = require('../functions/lib/src/journey/journey-day');
const {
    ...
    runTransaction: async (callback) => {
      ...
      const result = await callback({
        get: async (ref) => {
          ...
          if (ref.path.split('/').length % 2 === 0) return snapshot(ref);
          return {
            docs: [...documents.keys()]
              .filter(
                (path) =>
      ...
      for (const [path, document] of writes) documents.set(path, document);
      return result;
    },
  };
...
const dayRequest = { journeyId: 'current', dayNumber: 1, observedPhoneTimeZoneId: 'UTC' };
const origin = (id) => ({
  operationId: id,
  deviceId: 'test-device',
  recordedOnDeviceAt: { seconds: 1, nanoseconds: 0 },
});
const completeRequest = (id, practiceId = 'ReadScripture', revision = 0, isComplete = true) => ({
  ...dayRequest,
  practiceId,
  isComplete,
...
test('all 77 readings retain their full source text and actual translation', async () => {
  ...
  documents.get('users/owner/journeys/current').state = { status: 'Completed' };
  for (const day of manuscript.days) {
    ...
    assert.deepEqual(
      session.scripture.primaryPassage.paragraphs.flatMap((paragraph) => paragraph.runs),
...
test('opening a day is idempotent and never completes a practice', async () => {
  ...
  assert.equal(documents.get('users/owner/journeys/current/days/1').lastParticipantUpdateAt, null);
});
...
test('completion survives reopening, can be undone, and retries do not create extra revisions', async () => {
  ...
  const first = await completeJourneyPracticeForAccount('owner', request, database);
  assert.equal(first.completion.revision, 1);
  assert.deepEqual(await completeJourneyPracticeForAccount('owner', request, database), first);
  assert.equal(
    (await getJourneyDayForAccount('owner', dayRequest, database)).day.practices.readScripture
      .status,
    'Complete',
  );
  ...
  const session = await getJourneyDayForAccount('owner', dayRequest, database);
  assert.equal(session.day.practices.optionalPractices[0].completion.status, 'Complete');
...
test('unassigned practices, reused operation IDs, and stale completions cannot overwrite records', async () => {
  ...
  await completeJourneyPracticeForAccount('owner', completeRequest('first'), database);
  await assert.rejects(
    completeJourneyPracticeForAccount('owner', completeRequest('stale'), database),
    { code: 'aborted' },
  );
  await assert.rejects(
    completeJourneyPracticeForAccount('owner', completeRequest('first', 'Pray'), database),
    { code: 'already-exists' },
  );
});
...
test('reflection saving and completion remain independent, including an empty reflection', async () => {
  ...
  assert.equal(
    (await getJourneyDayForAccount('owner', dayRequest, database)).day.practices.reflect.status,
    'NotMarked',
  );
  await completeJourneyPracticeForAccount('owner', completeRequest('reflect', 'Reflect'), database);
  await saveJourneyReflectionForAccount(
    'owner',
    reflectionRequest('empty-writing', '', 'first-writing'),
    database,
  );
  const reopened = await getJourneyDayForAccount('owner', dayRequest, database);
  assert.equal(reopened.day.reflection.text, '');
  assert.equal(reopened.day.practices.reflect.status, 'Complete');
  assert.equal(
    documents.get('users/owner/journeys/current/writingRevisions/first-writing').text,
    'A private response',
  );
});
...
test('competing reflection saves preserve the account version and reject a stale replacement', async () => {
  ...
  const saved = await saveJourneyReflectionForAccount('owner', request, database);
  assert.deepEqual(await saveJourneyReflectionForAccount('owner', request, database), saved);
  await assert.rejects(
    saveJourneyReflectionForAccount(
      'owner',
  ...
  assert.equal(
    (await getJourneyDayForAccount('owner', dayRequest, database)).day.reflection.text,
    'Original response',
  );
});
...
test('future days, unreached days after early ending, and another account are inaccessible', async () => {
  const { database, documents } = createDatabase();
  await assert.rejects(
    getJourneyDayForAccount('owner', { ...dayRequest, dayNumber: 2 }, database),
    { code: 'failed-precondition' },
  );
  await assert.rejects(getJourneyDayForAccount('other', dayRequest, database), {
    code: 'not-found',
  });
  await assert.rejects(
    completeJourneyPracticeForAccount('other', completeRequest('other'), database),
    { code: 'not-found' },
  );
  ...
  await assert.rejects(
    saveJourneyReflectionForAccount(
      'owner',
      {
        ...
        target: { kind: 'DailyReflection', journeyId: 'current', dayNumber: 2 },
      },
      database,
    ),
    { code: 'failed-precondition' },
...
test('withdrawn or mismatched Scripture stays unavailable without substituting translations', async () => {
  const { database, documents } = createDatabase();
  documents.get(`bibleTextEditions/${bibleTextEditionId}`).releaseState.status = 'Withdrawn';
  const session = await getJourneyDayForAccount('owner', dayRequest, database);
  ...
  await completeJourneyPracticeForAccount('owner', completeRequest('physical-bible'), database);
...
test('four chosen practices are preserved and effective changes apply only to the assigned day', async () => {
  ...
  journey.initialOptionalPracticeIds = ['Movement', 'ServeOrEncourage', 'Gratitude', 'Worship'];
  ...
  assert.equal(
    (await getJourneyDayForAccount('owner', dayRequest, database)).day.practices.optionalPractices
      .length,
    4,
  );
  const second = await getJourneyDayForAccount('owner', { ...dayRequest, dayNumber: 2 }, database);
  assert.deepEqual(
    second.day.practices.optionalPractices.map((practice) => practice.practiceId),
...
test('request boundaries reject invalid dates, IDs, revisions, timestamps, and writing targets', () => {
  ...
  for (const request of [
    ...
    assert.throws(() => parseGetJourneyDayRequest(request), { code: 'invalid-argument' });
  }
  assert.throws(
    () =>
      parseCompleteJourneyPracticeRequest({
  ...
  assert.throws(
    () =>
      parseCompleteJourneyPracticeRequest({
        ...completeRequest('bad'),
        origin: { ...origin('bad'), recordedOnDeviceAt: { seconds: 1, nanoseconds: 1e9 } },
      }),
    { code: 'invalid-argument' },
  );
  assert.throws(
    () => parseSaveJourneyReflectionRequest(reflectionRequest('long', 'x'.repeat(10001))),
    { code: 'invalid-argument' },
  );
  assert.throws(
    () =>
