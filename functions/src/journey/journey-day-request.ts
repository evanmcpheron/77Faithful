...
const invalidRequest = () =>
	new HttpsError('invalid-argument', 'Reopen the day and try again.');
...
export const parseGetJourneyDayRequest = (
	input: unknown,
): IGetJourneyDayRequest => {
...
export const parseCompleteJourneyPracticeRequest = (
	...
	const practiceId = practices.find(
		(practice) => practice === request.practiceId,
	);
	if (
		!practiceId ||
	...
	return {
		...
		expectedRevisionId:
			request.expectedRevisionId === null
				? null
				: requireId(request.expectedRevisionId),
		text: request.text,
		origin: parseOrigin(request.origin),
	};
};
