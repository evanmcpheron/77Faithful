...
- For shipping Amplify features, check deployed Auth/Data definitions and indexes, explicit Cognito Lite configuration (or a documented Essentials requirement), authorization, private-data deletion, offline/sync evidence, correct environment outputs, and operational limits. Review scoped Lambda IAM and Storage/S3 permissions only when those capabilities exist. For Scripture integration, run the Scripture release audit and check approved passage assignments, verified source text, enabled fallback coverage, attribution/licensing, and unavailable-text behavior.
