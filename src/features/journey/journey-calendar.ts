...
export const getJourneyCalendarDate = (
	instant: Date,
	timeZoneId: string,
): TCalendarDate => {
...
export const getJourneyDayNumber = (
	startDate: string,
	calendarDate: string,
): number => {
	return (
		Math.round(
			(Date.parse(`${calendarDate}T00:00:00Z`) -
...
export const getJourneyEndTime = (
	...
	while (earliest < latest) {
		...
		if (getJourneyCalendarDate(new Date(midpoint), timeZoneId) < endDate)
			earliest = midpoint + 1;
