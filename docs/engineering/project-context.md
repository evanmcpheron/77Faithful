...
| AWS account/access, region, and Amplify backend environment identifiers          | **TBD — product owner must supply/confirm.** Verify a supported US region, deployment access, isolated development/preview/production resources, and Cognito email-delivery setup. No AWS resources are configured here. |
