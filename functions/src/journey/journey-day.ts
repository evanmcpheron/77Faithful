...
const assignOptionalPractices = (
  ...
  if (practiceIds.length === 3)
    return [assign(practiceIds[0]), assign(practiceIds[1]), assign(practiceIds[2])];
  return [assign(practiceIds[0]), assign(practiceIds[1])];
};

export const serializeTimestamp = (timestamp: IPersistedTimestamp): IPersistedTimestamp => ({
  seconds: timestamp.seconds,
  nanoseconds: timestamp.nanoseconds,
});

const serializeCompletion = (completion: IPracticeCompletion): IPracticeCompletion => ({
  ...completion,
  updatedAt: completion.updatedAt ? serializeTimestamp(completion.updatedAt) : null,
});

const serializeDay = (day: IJourneyDayDocument): IJourneyDayDocument => {
  const assigned = day.practices.optionalPractices;
  const serializeOptional = (practice: TAssignedOptionalPractices[number]) => ({
  ...
  const optionalPractices: TAssignedOptionalPractices =
    assigned.length === 4
      ...
      : assigned.length === 3
        ...
        : [serializeOptional(assigned[0]), serializeOptional(assigned[1])];
  return {
    ...day,
    practices: {
    ...
    intention: day.intention
      ? { ...day.intention, updatedAt: serializeTimestamp(day.intention.updatedAt) }
      : null,
    reflection: day.reflection
      ? { ...day.reflection, updatedAt: serializeTimestamp(day.reflection.updatedAt) }
      : null,
...
export const readJourneyDay = async (
  ...
  const journeyReference = database.doc(`users/${userId}/journeys/${request.journeyId}`);
  const journey = (await transaction.get(journeyReference)).data() as IJourneyDocument | undefined;
  if (!journey || journey.userId !== userId)
    throw new HttpsError('not-found', 'This journey isn’t available.');
  const today = getJourneyCalendarDate(new Date(), request.observedPhoneTimeZoneId);
  ...
  if (request.dayNumber > lastReachedDay)
    throw new HttpsError('failed-precondition', 'This day hasn’t begun yet.');
  const dayReference = journeyReference.collection('days').doc(String(request.dayNumber));
  ...
  const dayContentId = existingDay?.content.dayContentId ?? String(request.dayNumber);
  const content = (
    await transaction.get(versionReference.collection('days').doc(dayContentId))
  ...
  if (
    ...
    throw new HttpsError('unavailable', 'We couldn’t load this day’s guidance. Please try again.');
  }
  let optionalPracticeIds = journey.initialOptionalPracticeIds;
  if (!existingDay && journey.practiceScheduleRevision > 0) {
    const changes = await transaction.get(journeyReference.collection('practiceChanges'));
    const effectiveChanges = changes.docs
      ...
      .sort((left, right) => right.scheduleRevision - left.scheduleRevision);
    optionalPracticeIds = effectiveChanges[0]?.optionalPracticeIds ?? optionalPracticeIds;
  }
  ...
  const day: IJourneyDayDocument = existingDay ?? {
    ...
    calendarDate: addJourneyCalendarDays(journey.startDate, request.dayNumber - 1),
  ...
  return { day, content, dayReference, versionReference, isNew: !existingDay };
};
...
export const getJourneyDayForAccount = async (
  ...
  return database.runTransaction(async (transaction) => {
    const { day, content, dayReference, versionReference, isNew } = await readJourneyDay(
      transaction,
      database,
      userId,
      request,
    );
    const profile = (await transaction.get(database.doc(`users/${userId}`))).data() as
      IUserProfileDocument | undefined;
    const preferences = (
      await transaction.get(database.doc(`users/${userId}/preferences/current`))
    ...
    if (!translation)
      throw new HttpsError('failed-precondition', 'Choose a Bible translation in Settings.');
    const overview = (
      await transaction.get(
        versionReference.collection('weekOverviews').doc(String(day.weekNumber)),
      )
    ).data() as IFormationWeekOverviewDocument | undefined;
    const introduction = (
      await transaction.get(
        versionReference.collection('weekIntroductions').doc(String(day.weekNumber)),
      )
    ...
    const editionId: unknown = configuration?.bibleTextEditionIds?.[translation.bibleVersionId];
    const assignment = (
      await transaction.get(database.doc(`scriptureAssignments/${content.scriptureAssignmentId}`))
    ...
    if (typeof editionId === 'string' && /^[a-zA-Z0-9_-]{1,128}$/.test(editionId)) {
      const editionReference = database.doc(`bibleTextEditions/${editionId}`);
      const edition = (await transaction.get(editionReference)).data() as
        IBibleTextEditionDocument | undefined;
      const reading = (
        await transaction.get(
          editionReference.collection('assignmentTexts').doc(content.scriptureAssignmentId),
        )
      ...
      if (
        ...
        reading.scriptureAssignmentId === content.scriptureAssignmentId &&
        assignment &&
        reading.primaryPassage.passageId === assignment.primaryPassage.passageId &&
        (assignment.supportingPassage
          ? reading.supportingPassage?.passageId === assignment.supportingPassage.passageId
