		...
		const load = async () => {
			try {
				...
				session.current = {
					...
					deviceRevision:
						loadedSnapshot.devicePreferences?.revision ?? null,
				};
				...
				console.warn('Journey setup could not be loaded.', {
					errorCode,
				});
				setErrorMessage(
	...
	const save = async (changes: TJourneySetupChanges): Promise<boolean> => {
		...
		const savePromise = (async () => {
			try {
				...
				if (!isMounted.current || session.current !== currentSession)
					return false;
				currentSession.draftRevision =
					(currentSession.draftRevision ?? -1) + 1;
				currentSession.deviceRevision =
					(currentSession.deviceRevision ?? -1) + 1;
				return true;
			} catch (error) {
