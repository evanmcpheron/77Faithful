...
- Check authentication, session lifetime, account switching, and access boundaries against `AGENTS.md`. Distinguish repository configuration from deployed-state evidence; use the [Amplify/AWS security prompt](10-amplify-security-review.md) access cases when relevant.
...
- Distinguish public client configuration from deployment credentials/backend secrets. Inspect environment handling, transport, local caches/backups, and residual personal data after sign-out or deletion.
- Examine collection, logging, analytics, crash reporting, third-party payloads, and sharing defaults for unintended disclosure. Follow deletion through affected related records, files, caches, and copies.
...
Use existing non-fixing local/integration checks and synthetic accounts/data; redact findings and do not inspect real personal records. Return concrete exploit or disclosure paths, impact, corrections, and meaningful validation. Separate confirmed problems from hypotheses, unavailable evidence, and absent future features. Stop at the stated data flow; report verification limits instead of inferring production security.
