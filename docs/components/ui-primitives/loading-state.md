...
**Status:** New  
**Recommended path:**
`src/components/ui/loading-state/loading-state.component.tsx`

**Purpose**  
Simple loading state with optional text.
...
**Used in screens**  
Screen/modal loading.

**Composition**  
ActivityIndicator and Typography.
...
export interface ILoadingStateProps {
	children?: ReactNode;
	variant?: TLoadingStateVariant;
	disabled?: boolean;
	testID?: string;
	onPress?: () => void;
}

export const LoadingStateVariant = {
	Default: 'Default',
	Muted: 'Muted',
	Brand: 'Brand',
} as const;

export type TLoadingStateVariant =
	(typeof LoadingStateVariant)[keyof typeof LoadingStateVariant];
```
