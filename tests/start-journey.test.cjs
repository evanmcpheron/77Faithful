...
const createDatabase = () => {
			...
			for (let attempt = 0; attempt < 5; attempt++) {
				...
				const transaction = {
					...
					set: (reference, document) =>
						writes.push(() =>
							documents.set(reference.path, document),
						),
				...
				if (
					[...reads].some(
						([path, document]) => documents.get(path) !== document,
					)
				)
					continue;
	...
	for (let dayNumber = 1; dayNumber <= 77; dayNumber++) {
		...
		documents.set(
			`bibleTextEditions/web-edition/assignmentTexts/reading-${dayNumber}`,
			{
				scriptureAssignmentId: `reading-${dayNumber}`,
				bibleVersionId: 'Web',
				...
				primaryPassage: {
					...
					displayReference: 'Test reference',
					paragraphs: [
						{ runs: [{ text: 'Test fixture, not Scripture' }] },
					],
				},
				supportingPassage: null,
			},
	...
	for (let weekNumber = 1; weekNumber <= 11; weekNumber++)
		documents.set(
			`formationCourses/course/versions/v1/weekIntroductions/${weekNumber}`,
			{
				courseId: 'course',
				courseVersionId: 'v1',
				weekNumber,
...
const request = (operationId = 'attempt-1') => ({
	...
	review: {
		...
		reviewedStartDate: getJourneyCalendarDate(
			new Date(),
			'America/New_York',
		),
	},
...
const journeys = (documents) =>
	[...documents.keys()].filter((path) =>
		/^users\/owner\/journeys\/[^/]+$/.test(path),
	);
...
test('start creates an active journey and account preference, keeps draft, and returns explicit timestamp JSON', async () => {
	...
	assert.equal(
		result.details.journey.startDate,
		request().review.reviewedStartDate,
	);
	assert.equal(result.details.journey.timeZoneId, 'America/New_York');
	assert.deepEqual(result.details.journey.initialOptionalPracticeIds, [
		'Movement',
		'Gratitude',
	]);
	assert.equal(
	...
	assert.deepEqual(Object.keys(result.details.journey.createdAt), [
		'seconds',
		'nanoseconds',
	]);
	assert.equal(
...
test('same operation recovers its original result even after journey ended and draft changed', async () => {
	...
	assert.deepEqual(
		await startJourneyForAccount('owner', request(), database),
		first,
	);
...
test('midnight review change returns dates without creating a journey', async () => {
	const { database, documents } = createDatabase();
	const input = request();
	input.review.reviewedStartDate = addJourneyCalendarDays(
		input.review.reviewedStartDate,
		-1,
	);
	const result = await startJourneyForAccount('owner', input, database);
	assert.equal(result.outcome, 'ReviewChanged');
	assert.equal(
		result.review.reviewedStartDate,
		request().review.reviewedStartDate,
	);
...
for (const [name, mutate, reason] of [
	...
	[
		'missing content',
		(documents) =>
			documents.delete('formationCourses/course/versions/v1/days/77'),
		'ContentUnavailable',
	],
	[
		'withdrawn translation',
		(documents) => {
			documents.get('bibleTextEditions/web-edition').releaseState.status =
				'Withdrawn';
		},
		'ContentUnavailable',
	],
	[
		...
		(documents) =>
			documents.delete(
				'bibleTextEditions/web-edition/assignmentTexts/reading-77',
			),
		'ContentUnavailable',
	],
	[
		...
		(documents) => {
			...
			).choices.optionalPracticeIds = ['Movement', 'Movement'];
		},
		undefined,
	],
]) {
...
test('calendar follows the supplied current zone across midnight, DST and Day 77', () => {
	...
	assert.equal(
		new Date(
			getJourneyEndTime('2026-01-01', 'America/New_York'),
		).toISOString(),
...
for (const [currentZone, expectedOutcome] of [
	...
	test(`Day 77 uses the current phone zone in ${currentZone} after travel`, async (context) => {
		...
		initialRequest.review.reviewedStartDate = getJourneyCalendarDate(
			instant,
			'America/New_York',
		);
		...
		assert.equal(
			documents.get(existingPath).timeZoneId,
			'America/New_York',
		);
	});
}
...
test('starting after an elapsed period reconciles completion at the current phone zone boundary', async () => {
	...
	const first = await startJourneyForAccount('owner', request(), database);
	const existingPath = journeys(documents)[0];
	const oldStartDate = addJourneyCalendarDays(
		request().review.reviewedStartDate,
		-80,
	);
	documents.set(existingPath, {
	...
	const next = await startJourneyForAccount(
		'owner',
		request('next'),
		database,
	);
	...
	assert.equal(
		journeys(documents).filter(
			(path) => documents.get(path).state.status === 'Active',
		).length,
		1,
	);
});
...
test('starting motivation gets a journey revision and retains its source history', async () => {
	...
	documents.get('users/owner/journeySetupDrafts/current').startingMotivation =
		head;
...
test('the provisional course imports atomically and passes the real start content checks', async () => {
	...
	assert.equal(
		documents.get(`scriptureAssignments/${firstDay.scriptureAssignmentId}`)
			.displayReference,
		'John 15:1–11',
	);
	assert.equal(
		documents.get(`scriptureAssignments/${thirdDay.scriptureAssignmentId}`)
			.displayReference,
		'Psalm 23',
	);
	const beforeRetry = [...documents];
	assert.equal(
		(await seedProvisionalCourse(database, Timestamp.now())).outcome,
		'AlreadyImported',
	);
	assert.deepEqual([...documents], beforeRetry);
	const result = await startJourneyForAccount('owner', request(), database);
	assert.equal(result.outcome, 'Started');
	assert.deepEqual(result.details.journey.course, {
		courseId,
		courseVersionId,
	});
});
