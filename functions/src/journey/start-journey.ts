...
export const parseStartJourneyRequest = (
	input: unknown,
): IStartJourneyRequest => {
	if (
		!isRecord(input) ||
	...
	try {
		getJourneyCalendarDate(
			new Date(),
			input.review.observedPhoneTimeZoneId,
		);
		const reviewedDate = new Date(
			`${input.review.reviewedStartDate}T12:00:00Z`,
		);
		if (
			getJourneyCalendarDate(reviewedDate, 'UTC') !==
...
const requirePublishedCourse = async (
	...
	const editionId: unknown =
		configuration?.bibleTextEditionIds?.[bibleVersionId];
	const unavailable = () =>
		new HttpsError(
			'failed-precondition',
	...
	if (!isId(courseId) || !isId(courseVersionId) || !isId(editionId))
		throw unavailable();
	const versionReference = database.doc(
		`formationCourses/${courseId}/versions/${courseVersionId}`,
	);
	const version = (await transaction.get(versionReference)).data();
	const edition = (
		await transaction.get(database.doc(`bibleTextEditions/${editionId}`))
	).data();
	if (
	...
	const introductions = await transaction.get(
		versionReference.collection('weekIntroductions'),
	);
	...
	for (const snapshot of days.docs) {
		...
		const assignment = (
			await transaction.get(
				database.doc(
					`scriptureAssignments/${day.scriptureAssignmentId}`,
				),
			)
		).data();
		const reading = (
			await transaction.get(
				database.doc(
					`bibleTextEditions/${editionId}/assignmentTexts/${day.scriptureAssignmentId}`,
				),
			)
		).data();
		const isPassageReady = (
			passage: unknown,
			passageId: unknown,
		): boolean => {
			if (
			...
			return passage.paragraphs.every(
				(paragraph: unknown) =>
					...
					paragraph.runs.every(
						(run: unknown) => isRecord(run) && hasText(run.text),
					),
			);
		};
		if (
...
const getDetails = (
	...
	journey: {
		...
		course: {
			courseId: journey.course.courseId,
			courseVersionId: journey.course.courseVersionId,
		},
		...
		startingMotivation: journey.startingMotivation
			? {
					...
					updatedAt: {
						...
						nanoseconds:
							journey.startingMotivation.updatedAt.nanoseconds,
					},
				}
			: null,
		createdAt: {
			seconds: journey.createdAt.seconds,
			nanoseconds: journey.createdAt.nanoseconds,
		},
		updatedAt: {
			seconds: journey.updatedAt.seconds,
			nanoseconds: journey.updatedAt.nanoseconds,
		},
	},
...
export const startJourneyForAccount = async (
	...
	const lockReference = userReference
		.collection('journeyControl')
		.doc('current');
	...
	return database.runTransaction(async (transaction) => {
		...
		if (operation.exists)
			return operation.data()!.result as TStartJourneyResult;
		...
		const existingJourney = existing?.data() as
			IJourneyDocument | undefined;
		const completedAt = existingJourney
			? getJourneyEndTime(
					existingJourney.startDate,
					input.review.observedPhoneTimeZoneId,
				)
			: null;
		if (
		...
		const startDate = getJourneyCalendarDate(
			now.toDate(),
			input.review.observedPhoneTimeZoneId,
		);
		...
		const preferencesReference = userReference
			.collection('preferences')
			.doc('current');
		const preferences = (
			await transaction.get(preferencesReference)
		).data();
