...
Review only under `AGENTS.md`. Read `docs/PRODUCT_REQUIREMENTS.md`, relevant engineering decisions, and `docs/FORMATION_CONTENT_SPEC.md` when content/fixtures/versioning are affected. Inspect the implementation, affected routes/callers, tests, and available runtime evidence.
...
- Apply the shared navigation-review checklist in `AGENTS.md`: material flow changes need contract updates, Community stays Future unless explicitly enabled, and settled V1 requirements must be assessed separately from deferred Future questions and external setup prerequisites.
