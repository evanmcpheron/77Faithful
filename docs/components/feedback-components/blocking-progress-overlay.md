...
**Status:** New  
**Recommended path:**
`src/components/feedback/blocking-progress-overlay/blocking-progress-overlay.component.tsx`
...
```ts
export interface IBlockingProgressOverlayProps {
	visible?: boolean;
	title?: string;
	message?: string;
	children?: ReactNode;
...
Use only when interruption is risky.
